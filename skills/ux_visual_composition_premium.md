# UX Visual Composition — Premium Interfaces

**When to use this skill:** Use when the quality of the work depends on **art direction, hierarchy, restraint, imagery, and motion** rather than component count. Complements `skills/ux_tone_well_within.md` (voice and warmth) and `skills/ux_charting_patterns.md` (charting flows). Tone skill = *how it reads*; charting patterns = *what the product does*; **this skill** = *how it looks, moves, and holds attention*.

**Scope in this repo:** The app is **React Native + Expo** (not Next.js). Treat **Framer Motion** as *conceptual* unless the task is explicitly web; for native, prefer **layout, spacing, typography, and React Native Reanimated** (or Expo-supported animation) for the same motion goals. **Landing or marketing web** in this monorepo—if added later—can follow the full-bleed hero and Framer guidance literally. **Product surfaces** (calendar, entry, settings) default to the **Apps** and **Utility Copy** sections below, not hero/marketing patterns.

---

## Goal

Ship interfaces that feel **deliberate, premium, and current**. Default toward award-level composition: one big idea, strong imagery (where appropriate), sparse copy, rigorous spacing, and a small number of memorable motions.
...
- **Content plan:** hero, support, detail, final CTA (or for app surfaces: primary workspace, support, detail, primary action)
- **Interaction thesis:** 2–3 motion ideas that change the feel of the screen
...
- Prefer a full-bleed hero or full-canvas visual anchor **when the surface is marketing or onboarding**; for routine app UI, prefer calm full-screen layout without decorative heroes.
- Make the **brand or product name** the loudest text where branding matters.
...
- Treat the **first viewport** as a poster, not a document **when the screen is meant to sell or onboard**.
...
**Checks:** If the first viewport still works after removing the image, the image is too weak. If the brand disappears after hiding the nav, the hierarchy is too weak.
...
**Well Within:** Daily charting screens should read as **calm utility** (`ux_tone_well_within.md`), not campaign copy—unless the screen is onboarding or a dedicated education/marketing view.
...
- one entrance sequence in the hero (or primary screen)
- one scroll-linked, sticky, or depth effect
- one hover, reveal, or layout transition that sharpens affordance (on native: press, focus, or screen transition)

**Framer Motion** when available (typically web) for:
...
**React Native:** prefer **Reanimated** / layout animation APIs for analogous patterns; keep motion **fast, restrained, and consistent**.
...
- smooth on mobile
- fast and restrained
- consistent across the page or app
...
- Is the brand or product unmistakable in the first screen (when branding is in scope)?
- Is there one strong visual anchor (when imagery is in scope)?
